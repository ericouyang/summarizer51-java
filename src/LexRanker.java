...
public class LexRanker implements SummaryExtractor {
    ...
    private void init(Sentence[] sentences) {
        ...
        for (Sentence sentence : sentences) {
            ...
            for (String word : tf.keySet()) {
                Integer frequency = df.get(word);
                df.put(word, frequency == null ? 1 : frequency + 1);
            }
        }
        for (Entry<String, Integer> entry : df.entrySet()) {
            idf.put(entry.getKey(), Math.log((double) sentences.length / (double) entry.getValue()));
        }
    }
    ...
    private class SentenceNode extends Node<Sentence> {
        ...
        public double calculateRelationScore(Node<Sentence> other) {
            Map<String, Integer> thisTf = tfs.get(this);
            Map<String, Integer> otherTf = tfs.get(other);
